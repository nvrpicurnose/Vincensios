...
export function getSubscriptionInterval(currentUser){
	...
	const startDate = moment(new Date()).month(currentMonth).week(currentWeek+1).startOf('day').toISOString();
	const endDate = moment(new Date()).month(currentMonth).week(currentWeek+2).startOf('day').toISOString();
	return {
		startDate: startDate,
		endDate: endDate
	}
